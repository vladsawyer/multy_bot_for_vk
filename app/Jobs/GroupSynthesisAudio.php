...
class GroupSynthesisAudio implements ShouldQueue
{
    ...
    function send_speechKit_synthesis($txt, $user_id){
        ...
        $headers = ['Authorization: Api-Key ' . env('YANDEX_API_TOKEN')];
        $ch = curl_init();
    ...
    function vkApi_upload($user_id, $audio_file) {
        ...
        $response = $vk->docs()->getMessagesUploadServer(env('VK_TOKEN'), array(
            'peer_id' => $user_id,
            'type' => 'audio_message',
        ));
        ...
        $vk = new VKApiClient('5.103', VKLanguage::RUSSIAN);
        $parameter = $vk->docs()->save(env('VK_TOKEN'), array(
            'file' => $file,
        ));
